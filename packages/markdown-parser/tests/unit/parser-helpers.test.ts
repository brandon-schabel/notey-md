...
describe("parseListLine", () => {
  ...
  test("parses ordered list items with . and ) delimiters", () => {
    ...
    expect(res1).toEqual({ ordered: true, start: 1, delimiter: ".", content: "Item 1" });

    const res2 = parseListLine("2) Item 2");
    expect(res2).toEqual({ ordered: true, start: 2, delimiter: ")", content: "Item 2" });
  });
  ...
  test("handles leading spaces correctly (up to 3)", () => {
    ...
    expect(res2).toEqual({ ordered: true, start: 1, delimiter: ".", content: "Item with 3 spaces" });

    const res3 = parseListLine("    * Item with 4 spaces");
    expect(res3).toBeNull();
  });

  test("handles different start numbers for ordered lists", () => {
    const res1 = parseListLine("5. Item starting at 5");
    expect(res1).toEqual({ ordered: true, start: 5, delimiter: ".", content: "Item starting at 5" });

    const res2 = parseListLine("123456789. Large start number");
    expect(res2).toEqual({ ordered: true, start: 123456789, delimiter: ".", content: "Large start number" });
  });
  ...
  test("handles invalid start numbers for ordered lists", () => {
    ...
    const res2 = parseListLine("0. Starts at zero");
    expect(res2).toEqual({ ordered: true, start: 0, delimiter: ".", content: "Starts at zero" });

    const res3 = parseListLine("1.2. Starts decimal");
    expect(res3).toBeNull();
  });
  ...
  test("handles empty list item content", () => {
    ...
    expect(res2).toEqual({ ordered: true, start: 1, delimiter: ".", content: "" });
  });
  ...
  test("handles mixed spaces and tabs", () => {
    const res1 = parseListLine("  \t* Item with spaces and tab");
    expect(res1).toBeNull();
  });
  ...
  test("handles content with special characters", () => {
    ...
    expect(res2).toEqual({ ordered: true, start: 1, delimiter: ".", content: "Item with [link](url)." });
  });
  ...
  test("handles multiple spaces between bullet/number and content", () => {
    ...
    expect(res2).toEqual({ ordered: true, start: 1, delimiter: ".", content: "Multiple spaces" });
  });
});
