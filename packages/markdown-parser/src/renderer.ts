...
export function renderAstToHtml(node: MarkdownNode, isTop = true, idx = 0, count = 1): string {
  switch (node.type) {
    ...
    case "blockquote": {
      const inner = node.children.map((c, i) => renderAstToHtml(c, false, i, node.children.length)).join("")
    ...
    case "raw_html":
      return node.content
    case "link": {
      const inn = node.children.map((c, i) => renderAstToHtml(c, false, i, node.children.length)).join("")
